...
EnvelopePtr MakePing(int64_t time) {
  ...
  env->mutable_request()->mutable_ping()->set_src_time(time);
  return env;
}

EnvelopePtr MakePong(int64_t time) {
  auto env = std::make_unique<internal::Envelope>();
  env->mutable_response()->mutable_pong()->set_src_time(time);
  return env;
}
...
TEST(BrokerAndSenderTest, PingPong) {
  ...
  auto ping = thread([&]() {
    ...
    ASSERT_EQ(99, res->response().pong().src_time());
  });
  ...
  auto pong = thread([&]() {
    ...
    ASSERT_EQ(99, req->request().ping().src_time());
...
TEST(BrokerTest, CreateRedirection) {
  ...
  // Now we can receive the ping message
  {
    ...
    ASSERT_EQ(99, ping_req->request().ping().src_time());
  }
  ...
  // the beginning for the ping machine
  {
    ...
    ASSERT_EQ(99, pong_res->response().pong().src_time());
  }
}
